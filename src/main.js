  ...
  $("#searchForm").submit(function(event) {
    ...
    $.get(`https://bikeindex.org:443/api/v3/search?page=1&per_page=25&location= ${citySearch}&distance=${distanceSearch}&stolenness=proximity&access_token=process.env.API_KEY`).then(function(response) {
      ...
      for( let i = 0; i < response.bikes.length; i++)
      {
        ...
        $("#output").append(`<img src=${response.bikes[i].thumb} onerror="this.src='https://image.flaticon.com/icons/svg/13/13698.svg';">` + "<h3>" + response.bikes[i].title + "</h3>" + "<br>" + "<p>" + "<span id='infoHeaders'> Color: </span> " + response.bikes[i].frame_colors + "</p>" + "<br>" + "<p>" + "<span id='infoHeaders'> Date stolen: </span>" + timeConverter(response.bikes[i].date_stolen) + "</p>" + "<hr>");
      }
    }).fail(function(error) {
